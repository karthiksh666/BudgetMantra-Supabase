...
async def update_piggy_bank(body: PiggyBankUpdate, current_user: dict = Depends(get_current_user)):
    ...
    if existing.data:
        res = supabase.table("piggy_bank").update({"balance": body.balance, "updated_at": datetime.utcnow().isoformat()}).eq("user_id", current_user["id"]).execute()
    else:
        res = supabase.table("piggy_bank").insert({
            "id": str(uuid.uuid4()), "user_id": current_user["id"],
            "balance": body.balance, "updated_at": datetime.utcnow().isoformat()
        }).execute()
    return res.data[0]
